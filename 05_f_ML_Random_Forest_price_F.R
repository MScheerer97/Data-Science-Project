...
df_ML_price <- df_ML_raw %>%
  select(!contains("price_H") & !contains("price_K") & !contains("price_M") &
           !contains("price_B") & !contains("price_total") & !contains('distance_h') &
...
for (price_sel in prices) {
  ...
  predictors <- df_ML_price %>% 
    dplyr::select(-c(price_F, food, type, country, size)) %>% 
    colnames()

...
colnames(df_ML) <- str_replace_all(colnames(df_ML), "[[:punct:]]", "_")
colnames(df_ML) <- str_replace_all(colnames(df_ML), ">", "_")
...
nn_grid <- expand.grid(layer1 = c(15, 20, 25), layer2 = c(6, 8, 12), layer3 = c(3, 6, 9))
...
for(price in prices){
  ...
  for (fold in 1:length(fold_splits$splits)) {
    ...
    df_train <- analysis(fold_splits$splits[[fold]]) %>%
      select(-c(food, type, country, size, all_of(rem), all_of(distances_drop), date)) 
   ...
   df_train <- as.data.frame(apply(df_train, 2, norm_data))

    # obtain col number of price
    col_num <- which(colnames(df_train) == price)

    df_train_x <- df_train[, -col_num]
    df_train_y <- df_train[, col_num]
     
    # df_train_final <- cbind(df_train_y, df_train_x)
    
    # extract testing data
    df_test <- assessment(fold_splits$splits[[fold]]) %>%
      select(-c(food, type, country, size, all_of(rem), all_of(distances_drop), all_of(rem_lags), date)) 
    
    df_test_x <- as.data.frame(apply(df_test[, -col_num], 2, norm_data))
    df_test_y <- df_test[, col_num]
    y_max <- max(df_test_y)
    y_min <- min(df_test_y)
    ...
    # metrics
        ...
        MAPE <- MAPE(preds, pull(df_test_y))
        RMSE <- RMSE(preds, pull(df_test_y))
        MAE <- MAE(preds, pull(df_test_y))
  ...
  saveRDS(nn_tune_perf, paste0("models/NN/final_models/NN_full_", price, ".rds"))
...
saveRDS(results, paste0("models/NN/perf_results/NN_tuning_full_", price, ".rds"))
...
saveRDS(food_preds, paste0("models/NN/pred_results/NN_pred_full_", price, ".rds"))

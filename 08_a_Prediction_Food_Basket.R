...
# In this file, the price predictions for the upcoming week are made.
# Those predictions are used to generate the food basket.
...
for (i in 1:nrow(df_comb)) {
  ...
  for (markets_sel in markets) {
      ...
      ifelse(markets_sel == "price_H", "Hamburg",
             ifelse(markets_sel == "price_K", "Cologne",
                    ifelse(markets_sel == "price_B", "Berlin",
                           ifelse(markets_sel == "price_M", "Munich", markets_sel
                           )))))
    ...
    model_sel <- 
      ...
      ifelse(model_sel == "Support Vector Machines", "SVM", model_sel))))))
    
    
    
    # differentiate if we have BSTS model or not
...
# save data frame
saveRDS(df_pred_all, "output/food_basket_price_predictions.rds")

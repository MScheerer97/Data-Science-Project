...
df_ML_price <- df_ML_raw %>%
  select(!contains("price_F") & !contains("price_K") & !contains("price_M") &
          !contains("price_B") & !contains("price_total") & !contains('distance_f') &
           !contains('distance_k') & !contains('distance_m') & !contains('distance_b')) # all food
...
rf_grid <- expand.grid(
  trees = c(1000),  # Trying trees = 1500
  mtry = c(100),  #  round(ncol(df_ML_raw)/3)  # Trying mtry = 100 & mtry = 120
  #mtry = c(1:2),
  min_n = c(100)
)
...
for (price_sel in prices) {
  ...
  predictors <- df_ML_price %>% 
    dplyr::select(-c(price_H, food, type, country, size)) %>% 
    colnames()

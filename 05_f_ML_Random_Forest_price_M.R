...
df_ML_price <- df_ML_raw %>%
  select(!contains("price_F") & !contains("price_K") & !contains("price_H") &
           !contains("price_B") & !contains("price_total") & !contains('distance_f') &
           !contains('distance_k') & !contains('distance_h') & !contains('distance_b')) # all food
...
for (price_sel in prices) {
  ...
  predictors <- df_ML_price %>% 
    dplyr::select(-c(price_M, food, type, country, size)) %>% 
    colnames()

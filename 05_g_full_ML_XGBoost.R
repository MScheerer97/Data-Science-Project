...
xgb_grid <- expand.grid(
  ...
  tree_depth = c(6), # 3, 6, 9
  trees = c(500, 700), # 100, 500, 700, 1000
  learn_rate = c(0.01, 0.03), # 0.03, 0.01, 0.001
  # higher values seem to be better
  mtry = c(100, round(ncol(df_ML_raw)/3), 200, ncol(df_ML_raw)), # 2, 5, 10, 20, 50, 100, 150, 160, 200, 480
  # smaller values seem to be better
  min_n = c(1, 5, 10) # 1, 5, 10, 10, 20, 30, 40, 50, 60
)
...
saveRDS(xgb_best_param_final, 
        "models/XGBoost/perf_results/XGBoost_tuning_full_model.rds")
#saveRDS(xgb_perf_final, "xgb_perf_result.rds")
saveRDS(xgb_pred_final, 
        "models/XGBoost/pred_results/XGBoost_pred_full_model.rds")
...
#   inner_join(
#     xgb_best_param_final, by = c("model", "RMSE_min" = "RMSE")
#   )
# 
...
#     ## model specification
#   xgb_spec_final <- 
#     boost_tree(tree_depth = xgb_params_final$tree_depth, trees = xgb_params_final$trees,
#                learn_rate = xgb_params_final$learn_rate, mtry = xgb_params_final$mtry,
...
#   save_path <- paste0("XGBOOST_model_", price_sel, ".rds")

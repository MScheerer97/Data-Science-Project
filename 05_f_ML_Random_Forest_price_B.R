...
df_ML_price <- df_ML_raw %>%
  select(!contains("price_F") & !contains("price_K") & !contains("price_M") &
           !contains("price_H") & !contains("price_total") & !contains('distance_f') &
           !contains('distance_k') & !contains('distance_m') & !contains('distance_h')) # all food
...
for (price_sel in prices) {
  ...
  predictors <- df_ML_price %>% 
    dplyr::select(-c(price_B, food, type, country, size)) %>% 
    colnames()
